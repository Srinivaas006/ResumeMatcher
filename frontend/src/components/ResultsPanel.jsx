  ...
  return (
    <div className="space-y-4">
      ...
      {result.ats_keywords_missing?.length > 0 && (
        <div className="card p-5">
          <p className="section-title flex items-center gap-2">
            <Tag size={12} /> ATS Keywords Missing
          </p>
          <div className="flex flex-wrap gap-2">
            {result.ats_keywords_missing.map((kw, i) => (
              <span key={i} className="label-tag bg-signal-red_bg text-signal-red font-mono text-xs">
                {kw}
              </span>
            ))}
          </div>
          ...
          {result.missing_skills.map((s, i) => (
            <div key={i} className="p-3 bg-slate-faint rounded-xl">
              <div className="flex items-center justify-between mb-1.5">
                <span className="text-sm font-semibold text-ink">{s.skill}</span>
                <span className={`label-tag text-xs ${importanceBadge(s.importance)}`}>
                  {s.importance}
                </span>
              </div>
...
function ImprovementsTab({ result }) {
  return (
    <div className="card p-5 space-y-4">
      <p className="section-title flex items-center gap-2">
        <Lightbulb size={12} /> Suggested Improvements
      </p>
        ...
        <div key={i} className="p-4 bg-slate-faint rounded-xl space-y-2">
          ...
          {item.current && (
            <div>
              ...
              <p className="text-xs text-ink-muted font-mono bg-white border border-slate-light rounded-lg p-2 leading-relaxed">
                {item.current}
              </p>
            </div>
          )}
...
function InterviewTab({ result }) {
  return (
    <div className="card p-5 space-y-4">
      <p className="section-title flex items-center gap-2">
        <MessageSquare size={12} /> Likely Interview Questions
      </p>
